...
const componentsSpecDir = path.join(TEST_ROOT, 'lib');
...
task(':watch:components:spec', () => {
  watch(path.join(componentsDir, '**/*.ts'), [':build:components:ts:CJS']);
  watch(path.join(componentsSpecDir, '**/*.ts'), [':build:components:spec']);
});


/** Builds components typescript for tests (with CommonJS output). */
task(':build:components:ts:CJS', tsBuildTask(componentsDir, 'tsconfig-spec.json'));
task(':build:components:spec', [':build:components:ts:CJS'], tsBuildTask(componentsSpecDir, 'tsconfig-spec.json'));
